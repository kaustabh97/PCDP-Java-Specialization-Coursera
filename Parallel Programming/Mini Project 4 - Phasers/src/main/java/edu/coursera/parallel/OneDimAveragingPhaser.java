...
public final class OneDimAveragingPhaser {
    ...
    public static void runParallelFuzzyBarrier(final int iterations,
            final double[] myNew, final double[] myVal, final int n,
            final int tasks) {
        Phaser[] fuzzyPhaser = new Phaser[tasks];

        for(int i=0; i < fuzzyPhaser.length; i++){
            fuzzyPhaser[i] = new Phaser(1);
        }
        Thread[] threads = new Thread[tasks];
        ...
        for (int ii = 0; ii < tasks; ii++) {
            ...
            threads[ii] = new Thread(() ->{
                ...
                for (int iter = 0; iter < iterations; iter++) {
                    ...
                    int currPhase = fuzzyPhaser[i].arrive();

                    if (i-1 >= 0) fuzzyPhaser[i-1].awaitAdvance(currPhase);
                    if (i+1 < tasks) fuzzyPhaser[i+1].awaitAdvance(currPhase);
