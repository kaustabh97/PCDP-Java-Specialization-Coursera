...
public class MatrixMult {
    ...
    public static void parallelMatrixMultiply(Matrix a, Matrix b, Matrix c,
        ...
        final int rowChunk = (numRows + numRanks - 1)/numRanks;

        final int startRow = rank * rowChunk;
        int endRow = (rank+1) * rowChunk;
        if(numRows < endRow) endRow = numRows;

        mpi.MPI_Bcast(a.getValues(), 0,
                a.getNRows() * a.getNCols(), 0, mpi.MPI_COMM_WORLD);
        mpi.MPI_Bcast(b.getValues(), 0,
                b.getNRows() * b.getNCols(), 0, mpi.MPI_COMM_WORLD);
