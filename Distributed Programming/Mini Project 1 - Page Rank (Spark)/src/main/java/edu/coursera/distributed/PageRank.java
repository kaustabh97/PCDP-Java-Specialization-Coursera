    ...
    public static JavaPairRDD<Integer, Double> sparkPageRank(
        ...
        return  sites.join(ranks).
                flatMapToPair(i -> {
                    ...
                    Website linkEdge = i._2._1;
                    Double currRank = i._2._2;

                    List<Tuple2<Integer,Double>> contributions = new LinkedList<>();
                    Iterator <Integer> iter = linkEdge.edgeIterator();
                    while (iter.hasNext()){
                        final int targetSite = iter.next();
                        contributions.add(new Tuple2<>(targetSite, currRank/ (double) linkEdge.getNEdges()));
                    }
                    return contributions;
                }).reduceByKey(Double::sum)
                .mapValues(i -> 0.15 + 0.85*i);
    }
}
